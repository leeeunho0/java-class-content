...
public class MemberSystem {
	...
	public static void main(String[] args) {
		...
		// 프로그램이 실행되면 엑스버튼을 누를때까지는 계속 프로그램이 실행되야 된다.
		// 그래서 반복문을 이용해서 프로그램이 실행중!
		
		// 현재까지 우리가 만든 프로그램은 한번 실행하고 끝낸다.
		// 사용자가 계속 이용하고 싶어도 매번 다시 프로그램을 실
		// 무한반복
		// - 사용자가 프로그램 종료! 누르기 전까지는 계속 프로그램이 실행중이여야 된다.
		...
		for(;;) {
            ...
            // 숫자를 입력하고 문자를 그다음에 입력하면 건너뛰기 되버린다.
            // 그래서 숫자는 선택한 메뉴번호 변수가 저장하고 나머지는 지운다.
            input.nextLine();  // 초기화
            if(선택한메뉴번호 == 1) {
            ...
            // 2번을 누르면 로그인 메뉴입니다. 출력
            	...
            	// 로그인할 아이디를 입력받았으면 memberIds 를 검색해서
            	// 아이디가 없으면 로그인 성공!
            	//		 있으면 로그인 실패!
            	// 힌트 문자를 비교할때 사용하는 기능!
...
//            	}
            	for(String 잠깐저장 : memberIds) {
            		if(잠깐저장.equals(로그인할아이디)) {
            			System.out.println("로그인 성공!");
            			break; // 로그인 성공했으니 더이상 아이디 비교할 필요 X
            		}
            	}
            	// 2. 맞으면 로그인 성공 출력
            	//    틀리면 로그인 실패!
